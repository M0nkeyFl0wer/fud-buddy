...
export function PreferenceSelector({ onSubmit, isLoading }: PreferenceSelectorProps) {
  ...
  const [maxTravelMin] = useState<number>(() => {
    try {
      const raw = window.localStorage.getItem('fud_max_travel_min');
      const n = raw ? Number(raw) : 20;
      return Number.isFinite(n) ? Math.min(120, Math.max(5, n)) : 20;
    } catch {
      return 20;
    }
  });
      ...
      try {
        ...
        try {
          ...
          if (typeof data?.latitude === 'number' && typeof data?.longitude === 'number') {
            setOrigin({ lat: data.latitude, lon: data.longitude });
          }
  ...
  const handleSubmit = () => {
    try {
      window.localStorage.setItem('fud_max_travel_min', String(maxTravelMin));
    } catch {
      // ignore
    }
    onSubmit({
      ...
      <div className="sticky bottom-0 border-t bg-background/85 backdrop-blur supports-[backdrop-filter]:bg-background/70">
        <div className="max-w-lg mx-auto p-4">
          <Button
            ...
            disabled={isLoading}
            className="w-full py-7 text-2xl rounded-2xl"
            size="lg"
          >
